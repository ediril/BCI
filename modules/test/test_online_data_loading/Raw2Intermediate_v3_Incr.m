function [iData, IncrState] = Raw2Intermediate_v3_Incr( RawDataDir, file_read_count, IncrState)
...
%     file_stubs = cellfun(@(x, y) sprintf('%s%s',x, y), {N(ix).pre}, {N(ix).id}, 'un', false);
    ...
    if (nargin < 2) || (isempty(IncrState))
        file_stubs(file_read_count+1:end)=[]; 
        ...
        IncrState.FileNames(file_read_count+1:end) = []; 
        file_stubs(iFS(end)+file_read_count+1:end) = [];
    end
    ...
    IncrState.FileNames = file_stubs;
    IncrState.LastFileIndex = IncrState.LastFileIndex + file_read_count;
